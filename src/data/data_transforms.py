...
def atom23_list_to_atom27_list(batch, atom23_data_names, inplace=False):
    restype = batch["restype"].to(torch.long)
    assert (
        0 <= restype.min() <= restype.max() <= 4
...
def make_atom23_masks(na):
    ...
    na_aatype = na["restype"].to(torch.long).clone()
    ...
    restype_atom27_mask = torch.zeros([9, 27], dtype=torch.float32, device=na["restype"].device)
    for restype, restype_letter in enumerate(nc.restypes):
