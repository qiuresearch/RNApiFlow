...
### Torsion angles of nucleotide bases
# Format: The list for each residue type contains chi1, chi2, chi3, chi4 in
# this order (or a relevant subset from chi1 onwards). ALA and GLY don't have
# chi angles so their chi angle lists are empty.
chi_atoms_by_resname = {
    'A': [
        ["O4'", "C1'", "N9", "C8"],  # chi1, the last atom could be C4 too (or choose a more distant atom?)
    ],
    'G': [
        ["O4'", "C1'", "N9", "C8"],  # chi1, the last atom could be C4 too
    ],
    'C': [
        ["O4'", "C1'", "N1", "C2"],  # chi1, the last atom could be C6 too
    ],
    'U': [
        ["O4'", "C1'", "N1", "C2"],  # chi1, the last atom could be C6 too
    ],
...
# psi and chi angles. The atom positions are relative to the axis-end-atom of the corresponding
# rotation axis
# Start from origin (0, 0, 0) at C1' atom of the sugar ring.
# The x-axis is in direction of the rotation axis, and the y-axis
# is defined such that the dihedral-angle-definiting atom (the last entry in
# chi_angles_atoms above) is in the xy-plane (with a positive y-coordinate).
# format: [atomname, group_idx, rel_position]
# Atoms positions relative to the XXX rigid groups, defined by X, X, X, and X
# 0: 'sugar ring group' (XQ: and glycosidic bond)
...
atom23_names_by_resname = {
    'A': list(atom_names_by_resname["A"]) + [''],
    'G': list(atom_names_by_resname["G"]) + [],
    'C': list(atom_names_by_resname["C"]) + ['', '', ''],
    'U': list(atom_names_by_resname["U"]) + ['', '', ''],
}
...
def load_stereo_chemical_props() -> Tuple[Mapping[str, List[Bond]],
    ...
    """Load stereo-chemical properties for RNA from a text file.
    Returns:
        residue_bonds: A mapping from residue name to a list of Bond objects
            representing the bond lengths.
        residue_virtual_bonds: A mapping from residue name to a list of Bond
            objects representing the virtual bonds (derived from bond angles).
        residue_bond_angles: A mapping from residue name to a list of BondAngle
            objects representing the bond angles.
    """
...
# L=28 (27 unique atom names plus a possible OP3 for the first nucleotide)
atom28_names = [  
    "P",   # 0
    "OP1", # 1
    "OP2", # 2
...
atom28_indices_by_name = {atom_name: i for i, atom_name in enumerate(atom28_names)}
atom28_names_num = len(atom28_names)  # := 28

# This is the standard residue order when coding AA type as a number.
# Reproduce it by taking 3-letter AA codes and sorting them alphabetically.
restypes = ['A', 'G', 'C', 'U']
restype_indices = {restype: i for i, restype in enumerate(restypes)}
restype_indices["T"] = 3
restype_num = len(restypes)  # := 4.
unk_restype_index = restype_num  # Catch-all index for unknown restypes.
restypes_with_x = restypes + ['X']
restypes_with_x_g = restypes_with_x + ["-"]
restype_indices_with_x = {restype: i for i, restype in enumerate(restypes_with_x)}
restype_indices_with_x["T"] = 3
restype_indices_with_x_and_gap = {restype: i for i, restype in enumerate(restypes_with_x_g)}
restype_indices_with_x_and_gap["T"] = 3
for r in "WSMKRYBDHVN":  # map ambiguous bases into unknown
    restype_indices_with_x_and_gap[r] = unk_restype_index
rev_restype_indices_with_x_and_gap = {
    0: "A",
    1: "G",
    2: "C",
    3: "U",
...
map_id_to_out_restype = tuple(
    restypes_with_x_g.index(rev_restype_indices_with_x_and_gap[i])
...
def sequence_to_onehot(
    ...
    if sorted(set(mapping.values())) != list(range(num_entries)):
        raise ValueError(
            'The mapping must have values from 0 to num_unique_restypes-1 '
            'without any gaps. Got: %s' % sorted(mapping.values())
        )
    one_hot_arr = np.zeros((len(sequence), num_entries), dtype=np.int32)
    for ires, restype in enumerate(sequence):
        if map_unknown_to_x:
            if restype.isalpha() and restype.isupper():
                residx = mapping.get(restype, mapping['X'])
            else:
                raise ValueError(f'Invalid character in the sequence: {restype}')
        else:
            residx = mapping[restype]
        one_hot_arr[ires, residx] = 1
    return one_hot_arr
...
restypes_3to1 = {v: k for k, v in restypes_1to3.items()}

# Define a restype name for all unknown residues.
unk_restype = 'UNK'
resnames = [restypes_1to3[r] for r in restypes] + [unk_restype]
resnames_to_indices = {resname: i for i, resname in enumerate(resnames)}
...
def _make_standard_atom_mask() -> np.ndarray:
  ...
  mask = np.zeros([restype_num + 1, atom28_names_num], dtype=np.int32)
  for restype, resname in enumerate(restypes):
    resname = restypes_1to3[resname]
    for atom_name in atom_names_by_resname[resname]:
      mask[restype, atom28_indices_by_name[atom_name]] = 1
  return mask
...
def chi_angle_atom(idx: int) -> np.ndarray:
    """Define chi-angle rigid groups via one-hot representations.
    Args:
        idx: The index of the chi angle (1 or 2).
    """
    chi_angles_index = {}
    one_hots = []

    for restype, atom_groups in chi_atoms_by_resname.items():
        indices = [atom28_names.index(s[idx]) for s in atom_groups]
        indices.extend([-1]*(4-len(indices)))
        chi_angles_index[restype] = indices
        
    for r in restypes:
        res3 = restypes_1to3[r]
        one_hot = np.eye(atom28_names_num)[chi_angles_index[res3]]
        one_hots.append(one_hot)

    one_hots.append(np.zeros([4, atom28_names_num]))  # Add zeros for residue `X`.
...
chi_angles_atom_indices = [chi_atoms_by_resname[restypes_1to3[r]] for r in restypes]
_chi_angles_atom_indices = tree.map_structure(
    lambda atom_name: atom28_indices_by_name[atom_name], chi_angles_atom_indices
)
chi_angles_atom_indices = np.array([
    chi_atoms + ([[0, 0, 0, 0]] * (4 - len(chi_atoms)))
    for chi_atoms in _chi_angles_atom_indices]
)

# Mapping from (res_name, atom_name) pairs to the atom's chi group index
# and atom index within that group.
chi_groups_for_atom = collections.defaultdict(list)
for res_name, chi_angle_atoms_for_res in chi_atoms_by_resname.items():
...
def _make_rigid_transformation_4x4(ex, ey, translation):
    ...
    ez = np.cross(ex_normalized, ey_normalized)
    m = np.stack([ex_normalized, ey_normalized, ez, translation]).transpose()
...
# 5: the number of residue types (A, G, C, U, X)
...
# 10: the number of rigid groups (0-9)
restype_rigid_group_default_frame = np.zeros([5, 10, 4, 4], dtype=np.float32)

def _make_rigid_group_constants():
    """Fill the arrays above."""
    for restype, restype_letter in enumerate(restypes):
        resname = restypes_1to3[restype_letter]
        for atom_name, group_idx, atom_position in rigid_group_atom_positions[resname]:
            atomtype = atom28_indices_by_name[atom_name]
            ...
            atom23idx = atom23_names_by_resname[resname].index(atom_name)
    ...
    for restype, restype_letter in enumerate(restypes):
        ...
        if resname in ["A", "G"]:
            ...
            print(f"Warning: unknown residue {resname} in rigid group constants.")
            continue
        # backbone to backbone is the identity transform
        # group 0: sugar ring group (actually the four atoms defining the chi angle)
        ...
        # Rotation axes for the next frame start at (0,0,0) of the previous frame
        mat = _make_rigid_transformation_4x4(
        ...
        if chi_angles_mask[restype][0]:
            base_atom_names = chi_atoms_by_resname[resname][0]
...
def make_atom23_dists_bounds(overlap_tolerance=1.5,
    ...
    for restype, restype_letter in enumerate(restypes):
        resname = restypes_1to3[restype_letter]
        atom_list = atom23_names_by_resname[resname]
